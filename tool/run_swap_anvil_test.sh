#!/usr/bin/env bash
#
# Runs the swap Anvil integration test.
# Starts a local Anvil instance, runs the test, then stops Anvil.
#
# Usage:
#   bash tool/run_swap_anvil_test.sh
#
# Optional: set ANVIL_RPC_URL to override the default http://127.0.0.1:8545.
...
# Wait for Anvil to be ready (poll eth_chainId via host probe URL).
...
cd "$PROJECT_ROOT"
echo "Running swap integration test ..."
env \
  -u http_proxy -u https_proxy \
  flutter test \
    --dart-define=ANVIL_RPC_URL="$ANVIL_RPC_URL" \
    integration_test/swap_anvil_test.dart
EXIT_CODE=$?
exit "$EXIT_CODE"
